...
Какое-то скользящее окно реализовано [в библиотеке Pandas](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.rolling.html), однако автор плохо знаком с библиотекой Pandas, чтобы сказать, насколько универсальная и быстрая реализованная там реализация. К тому же всегда интереснее реализовать функционал самому, если есть такая возможность. Как альтернативу, можно задействовать библиотеку [Cython](https://cython.org/), однако скорость обработки все-равно будет ниже.
...
В общем, скользящее 2D окно позволяет проводить *периодические операции* на подматрице с каким-либо шагом. Операции сравнения, свертки, вычитания, умножения, применения цифрового фильтра и т.д. Здесь так же, как в [разделе один](#1d), создается вид нового массива с помощью формы и шагов.
...
*Форма* состоит из четырех слагаемых: три слагаемых такие же, как в [разделе один](#1d), а четвертое слагаемое — это количество вертикальных шагов скользящего окна `((a.shape[-2] - b.shape[-2]) // dy + 1,)` of the rolling 2D window. Здесь мы заменили:
```python
...
Пример скользящего 3D окна по 3D массиву: различные операции с вокселами (трехмерными пикселами) в трехмерных изображениях. В [тестовом примере](https://colab.research.google.com/drive/1Zru_-zzbtylgitbwxbi0eDBNhwr8qYl6#3d) ищутся совпадения малого 3D под-массива в большем, однако можно придумать и более сложные операции (свертка, цифровые фильтры, сравнения и т.д.).
...
```
  ...
  * Еще одна нетривиальная часть для `M < N`. Если `M < N` мы должны не только перевернуть одномерный 1D массив шагов скользящего окна, но и объединить его с одномерным массивом из единиц (шаг по высшим размерностям равен 1) для оставшихся размерностей `N-M`. Если `M == N`, то оставшиеся размерности равны нулю, и в этом случае конкатенация не требуется:
```python
    # flip 1D array of step sizes and concatenate it with remaining dimensions
    s = np.concatenate((np.ones(n-m, dtype=int), np.flip(d)))
```
