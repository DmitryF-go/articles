...
Рисунок — SAUNet состоит из двух частей: одна часть обрабатывает текстуры (texture stream); вторая часть обрабатывает формы (gated shape stream). В инкодере текстур сети U-Net используются блоки DenseNet-121 (смотрите выше [DenseNet](#DenseNet)), а в декодере в качестве словев сети U-Net — «блоки внимания» (dual attention decoder block).
